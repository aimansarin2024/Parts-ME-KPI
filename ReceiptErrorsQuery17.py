def sub_proc1():
    ...
    df.to_excel(writer, index=False, header=header, sheet_name='Sheet1')
    writer.close()
    df_pending_new = df # created a new variable df_pending_new and assigning the values of dataframe df in it. - Aiman
    ...
    if df_pending.empty:
        result4 = 0
            ...
            result4 = df_pending_new.groupby('PROCESS_FLAG_HD').agg(ERRORS_PENDING_TO_REPROCESS=('ERRORS_PENDING_TO_REPROCESS', 'sum')).reset_index().iloc[0]['ERRORS_PENDING_TO_REPROCESS'] # Aiman- Grouping the DataFrame df_pending_new by the column PROCESS_FLAG_HD then calculating the sum of the column ERRORS_PENDING_TO_REPROCESS for each group using the agg function. It then resets the index of the resulting DataFrame and finally retrieves the sum of ERRORS_PENDING_TO_REPROCESS for the first group.
