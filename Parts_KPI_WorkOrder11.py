def sub_proc1():
    ...
    file1="WorkOrder_Stage_{}".format(unstruct_toDate.strftime("%Y-%m-%d-%H-%M-%S").upper())+".xlsx"
    file2="WorkOrder_BaseT_{}".format(unstruct_toDate.strftime("%Y-%m-%d-%H-%M-%S").upper())+".xlsx"
    file3="WorkOrder_Unmatched_{}".format(unstruct_toDate.strftime("%Y-%m-%d-%H-%M-%S").upper())+".xlsx"
    file4="WorkOrder_Matched_{}".format(unstruct_toDate.strftime("%Y-%m-%d-%H-%M-%S").upper())+".xlsx"
    file5="WorkOrder_Merged_{}".format(unstruct_toDate.strftime("%Y-%m-%d-%H-%M-%S").upper())+".xlsx"
    file6="WorkOrder_Errors_Overdue_{}".format(unstruct_toDate.strftime("%Y-%m-%d-%H-%M-%S").upper())+".xlsx"
    file7="WorkOrder_WMS_{}".format(unstruct_toDate.strftime("%Y-%m-%d-%H-%M-%S").upper())+".xlsx"
    file8="WorkOrder_WMS_Unmatched_{}".format(unstruct_toDate.strftime("%Y-%m-%d-%H-%M-%S").upper())+".xlsx"
    file9="WorkOrder_WMS_Matched_{}".format(unstruct_toDate.strftime("%Y-%m-%d-%H-%M-%S").upper())+".xlsx"
    file10="WorkOrder_WMS_Merged_{}".format(unstruct_toDate.strftime("%Y-%m-%d-%H-%M-%S").upper())+".xlsx"
    file11="WorkOrder_Completed_but_NO_WIP_Issue_{}".format(unstruct_toDate.strftime("%Y-%m-%d-%H-%M-%S").upper())+".xlsx"
    file12="WorkOrder_Duplicate_Single_Piece_{}".format(unstruct_toDate.strftime("%Y-%m-%d-%H-%M-%S").upper())+".xlsx"
    ...
    query_result1=("""
        ...
        ,wt.trans_date_time
        ,wt.creation_date
        ,trunc(wt.creation_date) STG_DATE
        ,wt.last_update_date
        ,wt.reference_id
        ,wt.trans_quantity
        ,wt.email_sent_flg
        ...
        ,1 as TOTAL_COUNT
        from apps.gewip_apl_wo_trans_tbl wt
        where 1=1
        and wt.creation_date BETWEEN TO_DATE('"""+fromDate+"""', 'DD-MON-YYYY hh24:mi:ss')
        AND TO_DATE('"""+toDate+"""', 'DD-MON-YYYY hh24:mi:ss')
        order by wt.creation_date
    """)
    ...
    query_result2=("""
        ...
        ,null as EXCEEDS_2_HR
        ,1 as TOTAL_COUNT
        ,substr(mmt.transaction_reference,7,9) as internal_id
        FROM
        ...
        WHERE
        1=1
        AND mmt.transaction_type_id = 44 --in (10,13,12,15,16,18,19,22,27,1005,99,59,76,69,120,61,40,41,42,4) --= -- --in (31, 41) --in (31, 41,44,35) -= 44 --= 33 --
        AND mp.organization_code ='JEF'
    ...
    query_result3=("""
        ...
        ,wt.item_id
        ,wt.process_phase
        ,wt.process_status STATUS
        ,wt.transaction_type
        ,wt.organization_id
        ,wt.trans_date_time
        ,wt.creation_date
        ,trunc(wt.creation_date) STG_DATE
        ,wt.last_update_date-wt.creation_date REPROCESS_TIME
        ,wt.last_update_date
        ,wt.reference_id
        ,wt.trans_quantity
        ,wt.email_sent_flg
        ...
        from apps.gewip_apl_wo_trans_tbl wt
        where 1=1
        and wt.err_flag Not in ('P','I')
        and wt.creation_date BETWEEN TO_DATE('"""+fromDate3+"""', 'DD-MON-YYYY hh24:mi:ss')
    ...
    query_result4=("""
        ...
        ,LOCATION
        ,ACTIVITY_DATE_TIME
        ,DATE_FORMAT(ACTIVITY_DATE_TIME, '%Y-%m-%d') WMS_ACTIVITY_DATE_TIME 
        ...
        AND REFERENCE_TYPE in ('WIP Completion')
        order by ACTIVITY_DATE_TIME
    """)
    ## - Added code to include the WIP completion happened but no WIP issue
    query_result5=("""
        SELECT
            DISTINCT mp.organization_code, wdj.wip_entity_name JOB_Completed_But_WIP_Issue_Missing ---,mmt.transaction_type_id,
            ---mtt.transaction_type_name,mmt.TRANSACTION_SOURCE_ID
        FROM
            apps.mtl_material_transactions  mmt,
            apps.mtl_transaction_types      mtt,
            apps.mtl_parameters             mp,
        ...
        WHERE
            ...
            --AND mmt.transaction_id=8478631862
            AND mp.organization_code in ('JEF')    
            --AND msi.segment1 in ('239D4628G001')
            -- and wdj.wip_entity_name='5422303'
            --and nvl(wdj.STATUS_TYPE_DISP,'a')<>'Cancelled'
            --and not  exists (select 1 from apps.mtl_material_transactions mmtt where mmtt.transaction_source_id =wdj.wip_entity_id and transaction_type_id=35)
            and nvl(wdj.STATUS_TYPE_DISP,'a')<>'Cancelled'
            and exists (select 1 from apps.mtl_material_transactions mmtt where mmtt.transaction_source_id =wdj.wip_entity_id and transaction_type_id=44)
            and not  exists (select 1 from apps.mtl_material_transactions mmtt where mmtt.transaction_source_id =wdj.wip_entity_id and transaction_type_id=35)
            --AND mtt.transaction_type_name IN ('WIP Completion')
            --and mtt.transaction_type_name not IN ('WIP Issue')
            AND (trunc(wdj.creation_date) >= TO_DATE('"""+fromDate+"""', 'DD-MON-YYYY hh24:mi:ss'))-- between '25-MAR-2024' AND '03-APR-2024')
        order by wdj.wip_entity_name
    """)

    ## - Added code to include the Duplicate Single Piece WOs for same main WO with same qty  changed the query on 16/05/2024 which provided by Kaushik
    #query_result6=("""
    #    select ORIGINAL_WO,WORK_ORDER as Duplicate_Single_Piece_WOs,ASSEMBLY,START_QUANTITY,WO_CREATION_DATE,WO_SCHEDULED_DATE,WO_RELEASED_DATE from (
    #    select
    #    we.DESCRIPTION Original_WO,wip_entity_name Work_order,mst.segment1 Assembly,
    #    to_char(we.creation_date,'DD-MON-YYYY HH24:MI:SS' )  wo_creation_date
    #    ,to_char(we.SCHEDULED_START_DATE,'DD-MON-YYYY HH24:MI:SS' ) wo_scheduled_date
    #    ,to_char(we.DATE_RELEASED,'DD-MON-YYYY HH24:MI:SS' ) wo_released_date,
    #    count(we.wip_entity_name) over (Partition by we.DESCRIPTION,mst.segment1
    #    , we.START_QUANTITY,we.QUANTITY_COMPLETED,we.ATTRIBUTE1--,we.creation_date
    #    ,to_char(we.creation_date,'DD-MON-YYYY HH24' )
    #    ,to_char(we.SCHEDULED_START_DATE,'DD-MON-YYYY HH24' )
    #    ,to_char(we.DATE_RELEASED,'DD-MON-YYYY HH24' )  ) xrank,
    #    we.START_QUANTITY
    #    from apps.wip_discrete_jobs_v we
    #    ,apps.mtl_system_items_b mst
    #    where 1=1--and trunc(we.creation_date) between '01-mar-2024' and '31-mar-2024'
    #    --and we.wip_entity_name in ('GESP9423825','GESP9423828')
    #    ---and  we.description in ('RVR9409274-JEF')
    #    ---and we.wip_entity_name like 'GESP%'
    #    and we.PRIMARY_ITEM_ID= mst.inventory_item_id
    #    and we.ORGANIZATION_ID=mst.ORGANIZATION_ID
    #    and we.creation_date >= TO_DATE('"""+fromDate+"""', 'DD-MON-YYYY hh24:mi:ss')  -- >'01-Mar-2024 00:00:00'
    #    and we.ORGANIZATION_ID=202) where xrank=2
    #    order by 1,2                   
    #""")

    #changed the query on 16/05/2024 which provided by Kaushik
    query_result6=("""
        select ORIGINAL_WO,WORK_ORDER,ASSEMBLY,START_QUANTITY,WO_CREATION_DATE,WO_SCHEDULED_DATE,WO_RELEASED_DATE 
        from (select 
        we.DESCRIPTION Original_WO,wip_entity_name Work_order,mst.segment1 Assembly,
        to_char(we.creation_date,'DD-MON-YYYY HH24:MI:SS' )  wo_creation_date
        ,to_char(we.SCHEDULED_START_DATE,'DD-MON-YYYY HH24:MI:SS' ) wo_scheduled_date
    ...
    df5=pd.read_sql_query(query_result5,connection)
    header=list(df5.columns)
    print(file11)
    writer = pd.ExcelWriter(file11, engine='openpyxl')
    df5.to_excel(writer, index=False, header=header, sheet_name='Sheet1')
    writer.close()
    
    df6=pd.read_sql_query(query_result6,connection)
    header=list(df6.columns)
    print(file12)
    writer = pd.ExcelWriter(file12, engine='openpyxl')
    df6.to_excel(writer, index=False, header=header, sheet_name='Sheet1')
    writer.close()
   ...
   #     worksheet.conditional_format('G2:G{}'.format(len(result_df)+1), matched_cond_fmt)
    ...
    column_order = ['WMS_ACTIVITY_DATE_TIME', 'IN_STAGE', 'IN_EBS', 'REFERENCE_TYPE', 'WAREHOUSE', 'TOTAL_COUNT', 'NOT_IN_STAGE', 'NOT_IN_EBS', 'EXCEEDS_2_HR', 'RUN_TIME', 'QUANTITY']
    ...
    if not df_pending.empty:
        defect_percent['Errors not resolved within 24 hours'] = df_pending.groupby('STATUS').agg(ERRORS_PENDING_TO_REPROCESS=('ERRORS_PENDING_TO_REPROCESS', 'sum')).reset_index().iat[0, 1]
    else:
    ...
    if df_pending.empty:
        ...
        result4 = df_pending.groupby('STATUS').agg(ERRORS_PENDING_TO_REPROCESS=('ERRORS_PENDING_TO_REPROCESS', 'sum')).reset_index().iat[0, 1]
    ...
    # Convert the DataFrame to an HTML table
    html_table5 = df5.to_html(index=False)

    # Convert the DataFrame to an HTML table
    html_table6 = df6.to_html(index=False)
    ...
    usecols= []
    team = 'OPS Team'
    subject = "Parts_KPI_WorkOrder_{}"+toDate1
    emailbodytext = 'Please find attached Details for the Parts KPI Work Orders,  Request you to Report & Clear the  Defects Which are highlighted.'
    files = [file1,file2,file6,file7,file8,file9,file10,file11,file12]
    
    email_proc(sender_email, receiv_email, cc_email, passwd_email, unstruct_toDate, file0, usecols, team, attention, emailbodytext, files, subject, html_table1, html_table3, html_table4, html_table5, html_table6, result1, result2, result3, result4)


    # Construct the file pattern to match
    file_pattern = 'WorkOrder*.xlsx'
...
def email_proc(sender_email, receiv_email, cc_email, passwd_email, unstruct_toDate, file0, usecols, team, attention, emailbodytext, files, subject, html_table1, html_table3, html_table4, html_table5, html_table6, result1, result2, result3, result4):
    ...
    <html>
        <body>
            ...
            <p> </p>
            <p>KPI For Work Orders<p>
            <p>  1. Work Orders should get processed within 2 hours (Scale to EBS) :-  <b>{result1} % Work Orders are Processed after 2 hours</b> <p>
            <p>  2. Errors should be Less than 1% error                                      :-  <b>{result2} % Errors</b> <p>
            <p>  3. Errors should resolve within 24 hours                                   :-  <b>{result4} Record/s of Work Order/s is/are Pending to reprocess by Ops Team</b>  <p>
            <p> </p>
            <p>{emailbodytext}</p>
            <p> </p>
    ...
    html_body = MIMEText(html_content + html_table1 + html_table3 + html_table5 + html_table6, 'html')
